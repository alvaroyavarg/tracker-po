...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return NextResponse.json({
    data: { ...po, history: await getHistory(params.id), events: await getEvents(params.id) },
  });
}
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await getPo(params.id);
  ...
  const po = await updatePo(params.id, patch);

  if (statusChanged && po) {
    await addStatusChange(po.id, existing.status, patch.status, coerceText(body.statusNote));
  }
